...
* @include qmp_use.c
*/
void QMPool_put( QMPool * const me, void *b, bool Interrupt)
{
    ...
    */
    ...
    if(Interrupt){
    	...
    	uxSavedInterruptStatus = taskENTER_CRITICAL_FROM_ISR(); //Enter on critical section from ISR
    }
    ...
    if(Interrupt){
    	...
    	taskEXIT_CRITICAL_FROM_ISR( uxSavedInterruptStatus ); //Exit from critical section from ISR
    }
}
...
* @include qmp_use.c
*/
void *QMPool_get( QMPool * const me, unsigned short const margin,bool Interrupt )
{
    QFreeBlock *fb;
    UBaseType_t uxSavedInterruptStatus;
